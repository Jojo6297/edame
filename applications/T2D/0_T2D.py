...
dir = os.getcwd()
pdf1 = pd.read_csv(f"{dir}/{exp_num}_T2D.relative.abundance.csv", index_col='phyla')
...
pd.DataFrame(a).to_csv(f'Exp_{exp_num}_T2D_InitialAttractors.csv')
...
Int_mat2.to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_T2D_ESABONetworkIntMat.csv')
...
for x in test_max_num:
    ...
    NetwObIntMat.to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_{x}_T2D_FinalNetworkIntMat.csv')

    ## Uncomment: If want to export attractors of the Final network
    pd.DataFrame(AttrOb).to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_{x}_T2D_FinalAttractors.csv')

    ## Export the iteration scores
    pd.DataFrame(values).to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_{x}_T2D_Iterations.csv', index = False)

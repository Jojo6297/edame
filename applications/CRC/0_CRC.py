...
dir = os.getcwd()
pdf1 = pd.read_csv(f"{dir}/{exp_num}_CRC.relative.abundance.csv", index_col='phyla')
...
pd.DataFrame(a).to_csv(f'Exp_{exp_num}_CRC_InitialAttractors.csv')
...
Int_mat2.to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_CRC_ESABONetworkIntMat.csv')
...
for x in test_max_num:
    ...
    NetwObIntMat.to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_{x}_CRC_FinalNetworkIntMat.csv')

    ## Uncomment: If want to export attractors of the Final network
    pd.DataFrame(AttrOb).to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_{x}_CRC_FinalAttractors.csv')

    ## Export the iteration scores
    pd.DataFrame(values).to_csv(f'Exp_{exp_num}_{edge}_p{edge_p}_n{edge_n}_{x}_CRC_Iterations.csv', index = False)
